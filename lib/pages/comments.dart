...
import 'package:new_tv_360/models/Comment.dart';
...
Future<List<dynamic>> fetchComments(int id) async {
  try {
    var response = await http.get(Uri.parse("$WORDPRESS_URL/wp-json/wp/v2/comments?post=" + id.toString()));

    if (response.statusCode == 200) {
      return json
          .decode(response.body)
          .map((m) => CommentSection.fromJson(m))
          .toList();
    } else {
